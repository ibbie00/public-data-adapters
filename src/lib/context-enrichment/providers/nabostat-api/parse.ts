...
export function normalizeRows(payload: unknown): NabostatRawResult[] {
  ...
  return rows.filter((row): row is NabostatRawResult => Boolean(row && typeof row === "object"));
}

export function getStatusFromResponse(
  payload: unknown,
